...
async def create_plot(plot_data: PlotCreate, session: AsyncSession = Depends(get_session)):
    created_plot = await PlotDAO(session).create_plot(**plot_data.model_dump())
    if not created_plot:
...
async def get_plot(plot_id: int, session: AsyncSession = Depends(get_session)):
    plot = await PlotDAO(session).get_plot(plot_id)
    if not plot:
...
async def update_plot(plot_id: int, plot_data: PlotUpdate, session: AsyncSession = Depends(get_session)):
    updated_plot = await PlotDAO(session).update_plot(plot_id, **plot_data.model_dump())
    if not updated_plot:
        raise HTTPException(status_code=404, detail="Plot not found")
    return updated_plot
...
async def delete_plot(plot_id: int, session: AsyncSession = Depends(get_session)):
    deleted_plot = await PlotDAO(session).delete_plot(plot_id)
    if not deleted_plot:
        raise HTTPException(status_code=404, detail="Plot not found")
    return deleted_plot